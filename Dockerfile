FROM fredhutch/r-shiny-base:4.4.1
...
# RUN R --vanilla -e "install.packages('renv', repos='https://p3m.dev/cran/__linux__/jammy/latest/')"

# RUN R --vanilla -e "install.packages(c('readxl', 'arrow', 'feather', 'anndata', 'dichromat', 'dplyr', 'scattermore', 'DT', 'ggplot2', 'ggpubr', 'shiny', 'shinycssloaders', 'shinydashboard', 'shinyWidgets', 'reticulate', 'tibble', 'viridis', 'hrbrthemes', 'sccore', 'RColorBrewer', 'pals'), repos='https://p3m.dev/cran/__linux__/jammy/latest/')"

RUN R --vanilla -e "install.packages('systemfonts', repos='https://p3m.dev/cran/__linux__/jammy/latest/')"

RUN R --vanilla -e "install.packages('Matrix', repos='https://p3m.dev/cran/__linux__/jammy/latest/')"

RUN R --vanilla -e "install.packages(c('renv', 'ggpubr'), repos='https://p3m.dev/cran/__linux__/jammy/latest/')"
...
RUN R --vanilla -f check.R --args renv readxl arrow feather anndata dichromat dplyr scattermore DT ggplot2 ggpubr shiny shinycssloaders shinydashboard shinyWidgets reticulate tibble viridis hrbrthemes sccore RColorBrewer pals systemfonts
